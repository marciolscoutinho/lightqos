...
class QuantumOptimizer:
    ...
    async def _fuse_rotations(self, circuit: Circuit) -> Circuit:
        ...
        while i < len(gates):
            if i + 1 < len(gates):
                ...
                if (hasattr(current, 'type') and hasattr(next_gate, 'type') and
                    current.type in ["RX", "RY", "RZ"] and
                    current.type == next_gate.type and
                    hasattr(current, 'qubits') and hasattr(next_gate, 'qubits') and
    ...
    def create_optimization_report(
        ...
        improvement = (original_gates - optimized_gates) / original_gates * 100 if original_gates > 0 else 0
...
if __name__ == "__main__":
    import asyncio
    from lightqos.the_light.transformer import Gate, Circuit as TransCircuit
