...
class EMFPredictor:
    ...
    def _generate_predictions(
        ...
        for step in range(1, steps + 1):
            ...
            # Determine phase
            predicted_phase = self._predict_phase(
                predicted_fidelity,
                predicted_ergotropy
            )
            
            # Number of pairs (may decrease due to consumption)
            predicted_num_pairs = max(
                int(base_state.num_pairs * (1.0 - 0.005 * step)),
                0
            )
    ...
    def detect_anomalies(
        ...
        for i, state in enumerate(predictions):
            ...
            if state.fidelity < threshold:
                alerts.append(
                    f"Step {i+1}: Low fidelity ({state.fidelity:.2f}) - "
                    f"Recycling recommended"
                )
            ...
            if state.phase == ThermodynamicPhase.RADIATION:
                alerts.append(
                    f"Step {i+1}: Entering RADIATION phase - "
            ...
            if state.num_pairs < 10:
                alerts.append(
                    f"Step {i+1}: Low pair count ({state.num_pairs}) - "
                    f"Generation needed"
                )
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        for i, pred in enumerate(predictions):
            print(f"  t+{i+1}: F={pred.fidelity:.3f}, "
