    ...
    pub fn apply(&self, initial_state: &DMatrix<Complex64>) -> MultiTimeProcess {
        ...
        for (idx, step) in self.steps.iter().enumerate() {
            ...
            if let Some(ref memory) = self.memory {
                current_state =
                    memory.apply_memory_effect(&current_state, step.time, &process.states);
            }

            process.add_state(step.time as usize, current_state.clone());
        }
...
mod tests {
    ...
    fn test_multi_time_process() {
        ...
        process.add_state(1, state1.map(|x| nalgebra::Complex::new(x, 0.0)));
