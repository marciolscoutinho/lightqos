...
impl QuantumComb {
    ...
    pub fn to_process_tensor(&self) -> crate::process_tensor::ProcessTensor {
        let mut pt =
            crate::process_tensor::ProcessTensor::new(self.slot_dimensions[0], self.num_slots);

        for (idx, channel) in self.channels.iter().enumerate() {
            let step = crate::process_tensor::TemporalStep::new(idx as f64, channel.clone());
            pt.add_step(step);
        }
...
impl CausalOrder {
    ...
    pub fn is_consistent(&self) -> bool {
        // A causal order is consistent when all precedence relations are valid
        // and no relation can reach back to one of its own predecessors.
        for &(earlier, later) in &self.precedence {
            if earlier >= self.num_slots || later >= self.num_slots {
                return false;
            }
            ...
            if self.has_path(later, earlier, &mut visited) {
                return false;
            }
        }
...
impl LinkProduct {
    ...
    pub fn apply(comb1: &QuantumComb, comb2: &QuantumComb) -> Result<QuantumComb, String> {
