...
impl QuantumDriver for IonQDriver {
    ...
    async fn get_job_status(&mut self, job_id: Uuid) -> DriverResult<JobStatus> {
...

        {
            ...
            self.query_status(&job_id).await
        }
    }
...
mod tests {
    ...
    fn test_driver_creation() {
        let driver = IonQDriver::new("fake_api_key".to_string(), IonQBackend::Aria);
    ...
    async fn test_backend_listing() {
        let mut driver = IonQDriver::new("key".to_string(), IonQBackend::Simulator);
        ...
        // Verificar Aria
        let aria = backends.iter().find(|b| b.name == "aria-1").unwrap();
