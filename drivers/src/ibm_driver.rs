...
use crate::{
    BackendInfo, CircuitSubmission, DriverConfig, DriverError, DriverResult, ExecutionResult,
    JobStatus, QuantumDriver,
};
...
impl QuantumDriver for IBMQuantumDriver {
    ...
    async fn get_job_status(&mut self, job_id: Uuid) -> DriverResult<JobStatus> {
...

        {
            ...
            self.query_job_status(&job_id).await
        }
    }
...
mod tests {
    ...
    fn test_driver_creation() {
        let driver = IBMQuantumDriver::new("fake_token".to_string(), "ibm_brisbane".to_string());
    ...
    fn test_config() {
        ...
        let driver =
            IBMQuantumDriver::new("token".to_string(), "ibm_kyoto".to_string()).with_config(config);
    ...
    async fn test_backend_listing() {
        let mut driver = IBMQuantumDriver::new("token".to_string(), "ibm_brisbane".to_string());
        ...
        let brisbane = backends.iter().find(|b| b.name == "ibm_brisbane").unwrap();
    ...
    async fn test_job_submission() {
        let mut driver =
            IBMQuantumDriver::new("token".to_string(), "ibmq_qasm_simulator".to_string());
