...
def demo_emf_manager():
    ...
    pair_ids = []
    for i in range(100):
        pair_id = emf.generate_pair(qubit_a=i, qubit_b=i + 100, fidelity=0.95)
...
def demo_contract_manager():
    ...
    valid = manager.validate_execution(
        contract_id=relaxed_id, duration_ns=5_000_000, phase_error=0.05, fidelity=0.85
    )
...
def demo_shadow_collector():
    ...
    if not RUST_AVAILABLE:
        ...
        # Create collector
    collector = ShadowCollector(snapshots_per_shadow=100, target_confidence=0.95)
...
def benchmark_python_vs_rust():
    ...
    for _ in range(iterations):
        _ = 0.95 * 0.98  # Simple calculation
