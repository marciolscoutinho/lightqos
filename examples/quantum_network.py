...
if LIGHTQOS_AVAILABLE:
    ...
    pairs_distributed = protocol.distribute_pairs(
        links=[link_ab, link_bc, link_ca], pairs_per_link=5, use_emf_pool=True
    )
...
if LIGHTQOS_AVAILABLE:
    ...
    swap_result = swap_protocol.execute(path=route["path"], use_emf=True)
...
if LIGHTQOS_AVAILABLE:
    ...
    print("   Canal: Alice ↔ Bob")
    print(f"   Fidelidade do canal: {link_ab.fidelity * 100:.1f}%")

    # Executar BB84
    print("\n🚀 Executando BB84...")
    qkd_result = bb84.distribute_key(
        sender=alice, receiver=bob, key_length=key_length, use_efal_routing=True
    )
...
if LIGHTQOS_AVAILABLE:
    ...
    print(
        f"   Link degradado: fidelidade {link_bc.fidelity * 100:.1f}% → "
    ...
    print("   Novos limites temporais:")
    for node_name, contract in tlm_adjustments.items():
        print(
            f"   - {node_name}: {contract['max_duration_ns']}ns "
...
if LIGHTQOS_AVAILABLE:
    ...
    num_links = large_net.create_mesh_topology(nodes_per_connection=3, min_fidelity=0.85)
    ...
    throughput_test = large_net.benchmark_throughput(
        duration_seconds=10, protocol="epr_distribution"
    )
