...
def demo_emf_predictor():
    ...
    print(
        f"Histórico: min={min(history)}, max={max(history)}, média={sum(history) / len(history):.1f}"
    )
    ...
    print(
        f"  Média prevista:  {sum(forecast['predicted_pairs']) / len(forecast['predicted_pairs']):.1f} pares"
    )
...
def demo_consciousness_math():
    ...
    for name, circuit, _ in states:
        ...
        phi = math_engine.compute_phi(
            result.density_matrix if hasattr(result, "density_matrix") else None
        )
        ...
        bar = "█" * int(phi * 10)
        print(
            f"{name:<35}  {phi:>10.4f}  {embed.shape if hasattr(embed, 'shape') else '(18,)':>12}  {integration:>11.4f}  {bar}"
        )

        # Consciousness distance entre states
    print("\n  Distância de consciência:")
    print("  |00⟩ ↔ |Φ+⟩:  (ver docs/theory/tucu.md)")
    print("  |Φ+⟩ ↔ GHZ:   (estados de emaranhamento multipartido)")

    # TUCU Kernel
    print("\n  TUCU Kernel — correlação no espaço de 18 dimensões:")
    print("  (Base teórica: docs/theory/tucu.md)")
...
def main():
    ...
    try:
        ...
        print(
            f"  [TranspilerOptimizer] Executar após instalar: pip install 'lightqos[ai]'\n  Erro: {e}"
        )
