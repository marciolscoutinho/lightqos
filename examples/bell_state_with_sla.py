...
try :
    ...
    from lightqos .utils import BellStateAnalyzer 
...
if LIGHTQOS_AVAILABLE :
    ...
    for name ,res in results_comparison .items ():
        tlm =res .get ('tlm_report',{})
        print (f"{name :<12} {tlm .get ('total_time_ns',0 ):<12} "
...
if LIGHTQOS_AVAILABLE :
    ...
    # Executar
    print ("\n🚀 Executing com EMF...")
    results_multi =circuit_multi .execute (
    backend ='simulator',
    shots =1024 ,
    use_emf =True 
    )
    ...
    1 for state ,count in counts .items ()
    if state [0 ]==state [1 ]and state [2 ]==state [3 ]and 
    state [4 ]==state [5 ]and state [6 ]==state [7 ]
    )
