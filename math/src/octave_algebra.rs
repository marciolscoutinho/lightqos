...
impl OctaveAlgebra {
    ...
    /// Product of two states from different octaves
    pub fn octave_product(&self, n1: u8, n2: u8) -> Option<u8> {
        // Octave product: modulation
        if n1 < 1 || n1 > 10 || n2 < 1 || n2 > 10 {
            return None;
        }

        // Index sum (with wraparound)
        let result = ((n1 + n2 - 2) % 10) + 1;
        Some(result)
    }

    /// Resonance between octaves
    pub fn resonance(&self, n1: u8, n2: u8) -> f64 {
        if n1 < 1 || n1 > 10 || n2 < 1 || n2 > 10 {
            return 0.0;
        }

        let _oct1 = &self.octaves[(n1 - 1) as usize];
        let _oct2 = &self.octaves[(n2 - 1) as usize];

        // Resonance inversely proportional to the difference
    ...
    /// Transition between octaves (emission/absorption)
...
impl OctaveSpace {
    ...
    /// Central photon energy (J)
    pub fn photon_energy(&self) -> f64 {
        H * self.central_frequency()
    }

    /// Checks whether frequency is within this octave
...
pub enum OctaveName {
    ...
    Consciousness, // 512-1024 Hz (universal consciousness frequency)
    Unknown,
}
...
/// Harmonic structure das octaves
#[derive(Debug, Clone)]
pub struct HarmonicStructure {
    /// Harmonic coupling matrix (10×10)
...
impl HarmonicStructure {
    pub fn new() -> Self {
        ...
        // Diagonal: self-coupling = 1
        for i in 0..10 {
            coupling[i][i] = 1.0;
        }

        // Coupling between adjacent octaves
        for i in 0..9 {
            coupling[i][i + 1] = 0.5; // Upper octave
            coupling[i + 1][i] = 0.5; // Lower octave
        }

        // Harmonic coupling (2:1 ratio)
        for i in 0..8 {
            coupling[i][i + 2] = 0.25;
            coupling[i + 2][i] = 0.25;
        }
    ...
    /// Gets coupling between octaves n1 and n2
...
mod tests {
    ...
    fn test_resonance() {
        ...
        // Perfect resonance com si mesma
        let res = algebra.resonance(5, 5);
        assert_eq!(res, 1.0);

        // Lower resonance with adjacent octave
