...
// efal_tests.rs — EFAL Integration Tests — Ether Field Abstraction Layer verification
//
...
fn test_defect_allocation() {
    ...
    let defect_id = field
        .allocate_defect(GA3D::new(1.0, 1.0, 1.0), defect::DefectType::Qubit)
        .unwrap();

    assert!(field.defects.contains_key(&defect_id));

    // Attempts to allocate on an inertial plane, should fail
...
fn test_channel_creation() {
    ...
    // Creates two defects
    let dt_a = field
        .allocate_defect(GA3D::new(0.5, 0.5, 0.5), defect::DefectType::Qubit)
        .unwrap();

    let dt_b = field
        .allocate_defect(GA3D::new(1.5, 0.5, 0.5), defect::DefectType::Qubit)
        .unwrap();

    // Creates a channel between them
    let channel_id = field
        .create_channel(&dt_a, &dt_b, channel::ChannelType::Optical)
        .unwrap();
