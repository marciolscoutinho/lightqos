...
impl PACGuarantee {
    ...
    pub fn confidence_interval(&self) -> Option<(f64, f64)> {
        self.current_estimate
            .map(|est| (est - self.epsilon, est + self.epsilon))
    }
...
impl GuaranteedEstimator {
    ...
    fn update_variance(&mut self) {
        ...
        let sum_sq_diff: f64 = self.samples.iter().map(|&x| (x - mean) * (x - mean)).sum();
...
impl ConvergenceAnalyzer {
    ...
    pub fn has_converged(&self, threshold: f64) -> bool {
        ...
        let variance: f64 =
            window.iter().map(|&x| (x - mean) * (x - mean)).sum::<f64>() / window.len() as f64;
...
mod tests {
    ...
    fn test_confidence_interval() {
        ...
        assert!((ci.lower_bound - 0.4).abs() < 1.0e-12);
        assert!((ci.upper_bound - 0.6).abs() < 1.0e-12);
        assert!((ci.width() - 0.2).abs() < 1.0e-12);
    ...
    fn test_guaranteed_estimator() {
        ...
        // Add enough samples to satisfy the PAC bound.
        let required_samples = estimator.get_guarantee().required_samples;
