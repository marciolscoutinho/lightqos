...
use crate::hio::shadow_copy::{PauliOperator, PauliString, QuantumShadow};
...
impl Observable {
    ...
    pub fn from_pauli(name: String, pauli: PauliString) -> Self {
        ...
        let qubits = pauli
            ...
            .iter()
            .enumerate()
            .filter_map(|(i, &p)| if p != PauliOperator::I { Some(i) } else { None })
            .collect();
...
impl ViewManager {
    ...
    pub fn add_to_view(&mut self, view_id: Uuid, observable_id: Uuid) -> Result<(), String> {
        let view = self.views.get_mut(&view_id).ok_or("View not found")?;
    ...
    /// Measures view
    pub fn measure_view(&mut self, view_id: Uuid, shadow: &QuantumShadow) -> Result<(), String> {
        let view = self.views.get_mut(&view_id).ok_or("View not found")?;
    ...
    fn update_stats(&mut self) {
        ...
        let total_obs: usize = self.views.values().map(|v| v.observables.len()).sum();
...
impl ObservableFactory {
    ...
    pub fn pauli_x(qubit: usize, num_qubits: usize) -> Observable {
        ...
        Observable::from_pauli(format!("X_{}", qubit), PauliString::new(paulis))
    }
    ...
    pub fn pauli_y(qubit: usize, num_qubits: usize) -> Observable {
        ...
        Observable::from_pauli(format!("Y_{}", qubit), PauliString::new(paulis))
    }
    ...
    pub fn pauli_z(qubit: usize, num_qubits: usize) -> Observable {
        ...
        Observable::from_pauli(format!("Z_{}", qubit), PauliString::new(paulis))
    }
    ...
    pub fn surface_code_stabilizer(
        ...
        let mut obs =
            Observable::from_pauli(format!("Stab_{:?}", qubits), PauliString::new(paulis));
