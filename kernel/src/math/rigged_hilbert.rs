...
// rigged_hilbert.rs — Rigged Hilbert Spaces — Gel'fand triplets and Dirac formalism
//
...
/// Complex scalar (convenience alias)
pub type C64 = Complex64;

/// State vector in a finite-dimensional Hilbert space
...
/// Rigged Hilbert Space (Gel'fand Triplet: Φ ⊂ H ⊂ Φ')
///
/// Supports regular physical states (in Φ) and generalized states
/// such as the eigenkets of the position operator |x⟩ or momentum operator |p⟩.
#[derive(Debug, Clone)]
pub struct RiggedHilbertSpace {
    /// Hilbert space dimension H (may be ∞ — represented by truncation dimension)
    pub dimension: usize,
    /// Schwartz regularity index (0 = L², k>0 = more regular)
    pub regularity_index: u32,
    /// Space norm (2 = L² norm, ∞ = sup norm)
    pub norm_type: NormType,
}

/// Norm type for the space
...
impl RiggedHilbertSpace {
    /// Creates a new rigged Hilbert space
    ...
    /// Creates the Schwartz space S(ℝⁿ) — the core Φ
    ...
    /// Checks whether a state is normalized (⟨ψ|ψ⟩ = 1)
    ...
    pub fn inner_product(&self, bra: &StateVec, ket: &StateVec) -> C64 {
        assert_eq!(bra.len(), ket.len(), "Incompatible dimensions");
    ...
    /// Tensor product of two states (|ψ⟩ ⊗ |φ⟩)
    ...
    /// Projector |ψ⟩⟨ψ| (pure-state density)
    ...
    /// (approximate implementation through the density matrix diagonal)
    pub fn von_neumann_entropy(&self, rho: &DensityMatrix) -> f64 {
        // Uses approximate eigenvalues from the diagonal (valid for nearly diagonal states)
        let n = rho.len();
        let mut entropy = 0.0;
        for i in 0..n {
...
// ============================================================================
// DiracState — Ket and Bra in Dirac formalism
// ============================================================================

/// Dirac state in the rigged Hilbert space
///
/// Represents both regular kets |ψ⟩ ∈ Φ and functionals
/// generalized ⟨x| ∈ Φ' (Dirac distributions).
#[derive(Debug, Clone)]
...
impl DiracState {
    ...
    /// Uniform superposition state |+⟩^⊗n (Hadamard on all qubits)
...
// ============================================================================
// GeneralizedEigenfunction — generalized eigenfunction
// ============================================================================

/// Generalized eigenfunction of an operator (for continuous eigenvalues)
///
/// Example: eigenfunctions of the position operator Q|x⟩ = x|x⟩
/// or of the momentum operator P|p⟩ = p|p⟩
#[derive(Debug, Clone)]
pub struct GeneralizedEigenfunction {
    pub eigenvalue: f64,
    pub operator_name: String,
    /// Samples of the wave function at discrete grid points
    pub samples: Vec<C64>,
