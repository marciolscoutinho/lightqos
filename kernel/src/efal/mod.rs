...
// mod.rs — EFAL module — Ether Field Abstraction Layer public interface
//
...
pub struct EtherField {
    /// Fiber bundle over the laboratory space-time
...
impl EtherField {
    ...
    pub fn new(hardware_config: &HardwareConfig) -> Self {
        ...
        let fiber_bundle = FiberBundle::new(crate::math::fiber_bundle::SpaceTimeBase::default());
    ...
    pub fn create_channel(
        ...
        // Checks whether both defects exist
        if !self.defects.contains_key(source) || !self.defects.contains_key(target) {
            return Err(EFALError::DefectNotFound);
        }

        // Calculates the optimal path using PSER (Pressure-Sensitive Routing)
        let _path = self
            .topology
            .compute_pser_path(source, target, &self.geometry);
    ...
    pub fn allocate_defect(
        ...
        // Checks whether the position lies within the allowed inertial planes
    ...
    /// Dynamic real-time topology reconfiguration
