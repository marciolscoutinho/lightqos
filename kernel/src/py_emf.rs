...
impl PyEMFManager {
    ...
    fn get_pair(&self, id: String) -> PyResult<PyEntangledPair> {
        self.pairs.get(&id).cloned().ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyKeyError, _>(format!("Pair {} not found", id))
        })
    }

    /// Consumes pair (removes it)
    fn consume_pair(&mut self, id: String) -> PyResult<()> {
        self.pairs.remove(&id).ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyKeyError, _>(format!("Pair {} not found", id))
        })?;
    ...
    fn recycle(&mut self) -> usize {
        ...
        let to_recycle: Vec<String> = self
            .pairs
            .iter()
    ...
    fn avg_fidelity(&self) -> f64 {
        ...
        let sum: f64 = self.pairs.values().map(|p| p.fidelity).sum();
    ...
    fn get_statistics(&self, py: Python) -> PyResult<PyObject> {
        ...
        dict.set_item(
            "utilization",
            self.pairs.len() as f64 / self.max_pairs as f64,
        )?;
