...
impl EntanglementMetricsCalculator {
    ...
    pub fn calculate_fidelity(rho: &DMatrix<Complex<f64>>) -> Result<f64, String> {
    ...
    pub fn calculate_concurrence(rho: &DMatrix<Complex<f64>>) -> Result<f64, String> {
    ...
    pub fn calculate_entanglement_entropy(rho: &DMatrix<Complex<f64>>) -> Result<f64, String> {
    ...
    fn partial_trace_second_qubit(rho: &DMatrix<Complex<f64>>) -> DMatrix<Complex<f64>> {
    ...
    pub fn calculate_ergotropy(rho: &DMatrix<Complex<f64>>) -> Result<f64, String> {
        ...
        let mut sorted_eigenvalues: Vec<f64> = eigenvalues.iter().map(|c| c.re).collect();
        sorted_eigenvalues.sort_by(|a, b| b.partial_cmp(a).unwrap());

        // Passive-state energy
        let passive_energy: f64 = sorted_eigenvalues
            .iter()
            .enumerate()
    ...
    pub fn calculate_negativity(rho: &DMatrix<Complex<f64>>) -> Result<f64, String> {
    ...
    /// Partial transpose over the first qubit
    fn partial_transpose_first_qubit(rho: &DMatrix<Complex<f64>>) -> DMatrix<Complex<f64>> {
    ...
    fn matrix_sqrt(mat: &DMatrix<Complex<f64>>) -> Result<DMatrix<Complex<f64>>, String> {
    ...
    fn get_eigenvalues(mat: &DMatrix<Complex<f64>>) -> Result<Vec<Complex<f64>>, String> {
...
impl EMFPoolMetricsAggregator {
    ...
    pub fn aggregate_metrics(pairs: &[EntangledPairState]) -> EMFPoolMetrics {
...
mod tests {
    ...
    fn test_bell_state_fidelity() {
        ...
        let fidelity = EntanglementMetricsCalculator::calculate_fidelity(&phi_plus).unwrap();

        assert!(
            (fidelity - 1.0).abs() < 1e-6,
            "Bell-state fidelity must be 1.0"
        );
    }
    ...
    fn test_separable_state_concurrence() {
        ...
        let concurrence = EntanglementMetricsCalculator::calculate_concurrence(&rho).unwrap();

        assert!(
            concurrence < 0.1,
            "Separable state must have concurrence ~0"
        );
    }
