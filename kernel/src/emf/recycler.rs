...
use crate::emf::metrics::{
    EntangledPairState, EntanglementMetricsCalculator, ThermodynamicPhase,
...
impl RecyclingPolicy {
    ...
    pub fn should_recycle(&self, state: &EntangledPairState, pool_pressure: f64) -> bool {
...
impl EntanglementRecycler {
    ...
    pub fn process_queue(
        ...
        for pair_id in queued_pair_ids {
    ...
    fn recycle_pair(
        ...
        let result = match self.default_strategy {
            RecyclingStrategy::ErgotropyExtraction => self.extract_ergotropy(pair_id, state),
            ...
            RecyclingStrategy::Discard => self.discard_pair(pair_id, state),
        };
    ...
    fn extract_ergotropy(&self, pair_id: Uuid, state: &EntangledPairState) -> RecyclingResult {
    ...
    fn purify_pairs(
        ...
        let partner_pair = pool_pairs
            ...
            .filter(|(id, s)| {
                **id != pair_id
                    && s.fidelity < 0.9
                    && s.fidelity > 0.5
                    && ThermodynamicPhaseClassifier::classify_phase(s)
                        == ThermodynamicPhase::Degradation
            })
            .next();
        ...
        if let Some((partner_id, partner_state)) = partner_pair {
            ...
            // Dissipated entropy
            let dissipated_entropy =
                state.entanglement_entropy + partner_state.entanglement_entropy;
    ...
    fn distill_entanglement(
        ...
        let mut candidate_pairs: Vec<(Uuid, &EntangledPairState)> = pool_pairs
            .iter()
            .filter(|(id, s)| **id != pair_id && s.fidelity > 0.5 && s.fidelity < 0.85)
            .map(|(id, s)| (*id, s))
        ...
        if candidate_pairs.len() >= self.config.min_pairs_for_distillation {
            ...
            let avg_fidelity: f64 = candidate_pairs.iter().map(|(_, s)| s.fidelity).sum::<f64>()
                / candidate_pairs.len() as f64;

            // Distillation improves fidelity
            let distilled_fidelity = (avg_fidelity + 0.2).min(0.99);

            // Combined ergotropy
            let total_ergotropy: f64 = candidate_pairs.iter().map(|(_, s)| s.ergotropy).sum();

            let extracted_ergotropy = total_ergotropy * 0.3; // 30% extracted

            // Total entropy
            let total_entropy: f64 = candidate_pairs
                .iter()
                .map(|(_, s)| s.entanglement_entropy)
                .sum();

            let recycled_ids: Vec<Uuid> = candidate_pairs.iter().map(|(id, _)| *id).collect();
    ...
    /// Simple discard
    fn discard_pair(&self, pair_id: Uuid, state: &EntangledPairState) -> RecyclingResult {
        RecyclingResult {
            recycled_pairs: vec![pair_id],
...
impl EntanglementLifecycleManager {
    ...
    pub fn update_phase_counts(&mut self, pool_pairs: &HashMap<Uuid, EntangledPairState>) {
...
mod tests {
    ...
    fn test_lifecycle_manager() {
        let recycler =
            EntanglementRecycler::new(RecyclingPolicy::Balanced, RecyclingStrategy::Purification);
