...
impl NetworkLink {
    pub fn new(source: Uuid, target: Uuid, distance_km: f64, base_fidelity: f64) -> Self {
    ...
    ///
    /// `current_fidelity` already represents the physical quality of this link,
    /// including calibration, noise and distance effects. Route-level fidelity
    /// is computed by composing the per-link fidelities, so applying another
    /// exponential distance decay here would double-penalize multi-hop routes.
    pub fn expected_fidelity(&self) -> f64 {
        self.current_fidelity.clamp(0.0, 1.0)
    }
}
...
impl PSERRouter {
    ...
    pub fn calculate_route(&mut self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
        ...
        let route = match self.metric {
            RoutingMetric::MinHops => self.dijkstra_shortest_path(request)?,
            RoutingMetric::MaxFidelity => self.highest_fidelity_path(request)?,
            RoutingMetric::MinLatency => self.lowest_latency_path(request)?,
            RoutingMetric::MinCost => self.lowest_cost_path(request)?,
            RoutingMetric::LoadBalance => self.load_balanced_path(request)?,
            RoutingMetric::PressureGradient => self.pressure_gradient_path(request)?,
        };
    ...
    fn dijkstra_shortest_path(&self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
        ...
        while current != source {
            path.push(current);
            current = *predecessors.get(&current).ok_or("Invalid path")?;
        }
        path.push(source);
        path.reverse();
    ...
    fn highest_fidelity_path(&self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
        ...
        while current != source {
            ...
            current = *predecessors
                .get(&current)
                .ok_or_else(|| "missing predecessor while reconstructing route".to_string())?;
        }
        path.push(source);
        path.reverse();
    ...
    fn lowest_latency_path(&self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
    ...
    fn lowest_cost_path(&self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
        // Cost = distance + fidelity degradation + swapping overhead
        self.dijkstra_shortest_path(request) // Simplified
    }

    /// Load-balanced route
    fn load_balanced_path(&self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
        // Avoid nodes with high local pressure
        self.dijkstra_shortest_path(request) // Simplified
    }

    /// Route following pressure gradient (TUCU)
    fn pressure_gradient_path(&self, request: &RoutingRequest) -> Result<CalculatedRoute, String> {
    ...
    fn build_route(&self, request_id: Uuid, path: Vec<Uuid>) -> Result<CalculatedRoute, String> {
        if path.len() < 2 {
    ...
    fn update_stats(&mut self, route: &CalculatedRoute) {
        ...
        self.stats.avg_fidelity =
            (self.stats.avg_fidelity * (n - 1.0) + route.expected_fidelity) / n;
        self.stats.avg_latency_ms =
            (self.stats.avg_latency_ms * (n - 1.0) + route.expected_latency_ms) / n;
    }
...
mod tests {
    ...
    fn node_id_by_name(topology: &NetworkTopology, name: &str) -> Uuid {
        topology
            .nodes
            .values()
    ...
    fn test_dijkstra_routing() {
        ...
        let source = node_id_by_name(&topology, "A");
        let destination = node_id_by_name(&topology, "D");
