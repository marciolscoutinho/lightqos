...
use crate::emf::recycler::{
    EntanglementRecycler, RecyclerConfig, RecyclingPolicy, RecyclingStrategy,
};

use crate::emf::pser_routing::{NetworkTopology, PSERRouter, RoutingMetric, RoutingRequest};
...
impl EntanglementPool {
    ...
    pub fn with_config(config: PoolConfig) -> Self {
        // Create recycler
        let recycler =
            EntanglementRecycler::new(RecyclingPolicy::Adaptive, RecyclingStrategy::Purification)
    ...
    pub fn create_pair(&mut self, density_matrix: DMatrix<Complex<f64>>) -> Result<Uuid, String> {
    ...
    pub fn allocate_pair(&mut self, min_fidelity: f64) -> Option<Uuid> {
        ...
        pairs
            .iter()
            .filter(|(_, state)| {
                state.fidelity >= min_fidelity && state.reuse_count < 5 // Reuse limit
            })
            .max_by(|(_, a), (_, b)| a.fidelity.partial_cmp(&b.fidelity).unwrap())
            .map(|(id, _)| *id)
    }
    ...
    pub fn purify_pairs(&mut self, pair_a: Uuid, pair_b: Uuid) -> Result<Uuid, String> {
        let mut pairs = self.pairs.write().unwrap();

        let state_a = pairs.get(&pair_a).ok_or("Pair A not found")?.clone();

        let state_b = pairs.get(&pair_b).ok_or("Pair B not found")?.clone();

        // Entanglement purification protocol.
        //
        // We model pair purification as a Bell-diagonal recurrence step:
        //
        // F' = (F1 * F2) / (F1 * F2 + ((1 - F1) * (1 - F2)) / 9)
        //
        // This captures the intended behavior of consuming two noisy Bell pairs
        // to obtain one pair with higher Bell-state fidelity.
        let f1 = state_a.fidelity.clamp(0.0, 1.0);
        let f2 = state_b.fidelity.clamp(0.0, 1.0);

        let numerator = f1 * f2;
        let denominator = numerator + ((1.0 - f1) * (1.0 - f2)) / 9.0;

        if denominator <= f64::EPSILON {
            return Err("Purification denominator is numerically zero".to_string());
        }

        let purified_fidelity = (numerator / denominator).clamp(0.0, 0.99);
        ...
        // Bell-diagonal density matrix with exact target Bell fidelity.
        let mut purified_density = DMatrix::zeros(4, 4);
        let orthogonal_weight = (1.0 - purified_fidelity) / 3.0;

        purified_density[(0, 0)] =
            Complex::new(purified_fidelity / 2.0 + orthogonal_weight / 2.0, 0.0);
        purified_density[(1, 1)] = Complex::new(orthogonal_weight, 0.0);
        purified_density[(2, 2)] = Complex::new(orthogonal_weight, 0.0);
        purified_density[(3, 3)] =
            Complex::new(purified_fidelity / 2.0 + orthogonal_weight / 2.0, 0.0);

        purified_density[(0, 3)] =
            Complex::new(purified_fidelity / 2.0 - orthogonal_weight / 2.0, 0.0);
        purified_density[(3, 0)] =
            Complex::new(purified_fidelity / 2.0 - orthogonal_weight / 2.0, 0.0);
