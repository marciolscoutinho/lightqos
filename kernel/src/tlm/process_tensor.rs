...
impl ProcessTensorCache {
    ...
    fn compute_hash(system_dim: usize, memory_depth: usize, temperature: f64) -> u64 {
...
impl NonMarkovianityMetrics {
    ...
    pub fn blp_measure(states: &[DMatrix<Complex<f64>>], times: &[f64]) -> f64 {
    ...
    fn trace_distance(rho1: &DMatrix<Complex<f64>>, rho2: &DMatrix<Complex<f64>>) -> f64 {
        // D(ρ₁, ρ₂) = (1/2) Tr|ρ₁ - ρ₂|
        let diff = rho1 - rho2;
    ...
    pub fn rhp_measure(process_tensor: &ProcessTensor) -> f64 {
        ...
        backflow_events.iter().map(|(_, strength)| strength).sum()
    }
}
...
mod tests {
    ...
    fn test_builder_heom() {
        let builder = ProcessTensorBuilder::new(1, 5).with_temperature(300.0);
