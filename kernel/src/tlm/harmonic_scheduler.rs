...
use crate::tlm::contract::{ContractManager, TemporalContract};
use crate::tlm::process_tensor::ProcessTensorCache;
use crate::tlm::snapshot::{Operation, SnapshotManager};
...
impl ElectromagneticOctave {
    ...
    pub fn from_frequency(freq_hz: f64) -> Self {
        if freq_hz < 1e0 {
            ...
            Self::Heat
        } else if freq_hz < 1e12 {
            Self::Infrared
        } else if freq_hz < 1e15 {
            Self::Visible
        } else if freq_hz < 1e18 {
            Self::Ultraviolet
        } else if freq_hz < 1e21 {
            Self::XRay
        } else if freq_hz < 1e24 {
            Self::Gamma
        } else {
            Self::Consciousness
        }
...
impl Ord for QuantumTask {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        match self.priority.cmp(&other.priority) {
            Ordering::Equal => match (self.deadline_ns, other.deadline_ns) {
...
impl HarmonicScheduler {
    ...
    fn execute_task(&mut self, task: QuantumTask) -> TaskExecutionResult {
        ...
        let contract_met = if let Some(ref contract) = task.contract {
            let results = self
                .contract_manager
                .validate_temporal_all(duration_ns, None);
        ...
        if total_completed > 0 {
            self.stats.avg_task_duration_ns =
                (self.stats.avg_task_duration_ns * (total_completed - 1) + duration_ns)
                    / total_completed;
        }
    ...
    fn create_task_snapshot(&mut self, task: &QuantumTask) {
        ...
        self.snapshot_manager
            .create_snapshot(format!("before_{}", task.name), state_vector, 1);
    }
...
mod tests {
    ...
    fn test_octave_from_frequency() {
        assert_eq!(
            ElectromagneticOctave::from_frequency(5e2),
            ElectromagneticOctave::Magnetism
        );
        assert_eq!(
            ElectromagneticOctave::from_frequency(5e13),
            ElectromagneticOctave::Visible
        );
        assert_eq!(
            ElectromagneticOctave::from_frequency(5e25),
            ElectromagneticOctave::Consciousness
        );
    }
    ...
    fn test_harmonic_resonance() {
        ...
        let task1 = QuantumTask::new("t1".to_string(), vec![]).with_priority(200);
        let task2 = QuantumTask::new("t2".to_string(), vec![]).with_priority(200);
